...
/**
 * Finder mapping a directory of local resources. Those resources have representations accessed by the file system, the
 * class loaders or other URI accessible protocols. Here is some sample code illustrating how to attach a directory to a
 * router:<br>
 * 
 * <pre>
 ...
 * </pre>
 * 
 * An automatic content negotiation mechanism (similar to the one in Apache HTTP server) is used to select the best
 * representation of a resource based on the available variants and on the client capabilities and preferences.<br>
 * <br>
 * The directory can be used in read-only or modifiable mode. In the latter case, you just need to set the "modifiable"
 * property to true. The currently supported methods are PUT and DELETE.<br>
 * <br>
 * When no index is available in a given directory, a representation can be automatically generated by the
 * {@link #getIndexRepresentation(Variant, ReferenceList)} method, unless the "listingAllowed" property is turned off.
 * You can even customize the way the index entries are sorted by using the {@link #setComparator(Comparator)} method.
 * The default sorting uses the friendly Alphanum algorithm based on David Koelle's <a
 * href="http://www.davekoelle.com/alphanum.html">original idea</a>, using a different and faster implementation
 * contributed by Rob Heittman.<br>
 * <br>
 * Concurrency note: instances of this class or its subclasses can be invoked by several threads at the same time and
 * therefore must be thread-safe. You should be especially careful when storing state in member variables.
 * 
 * @see <a href="http://wiki.restlet.org/docs_2.1/374-restlet.html">User Guide - Serving static files</a>
...
public class SkysailDirectory extends Finder {
     ...
     * <br>
     *            If you serve files from the file system, use file:// URIs and make sure that you register a FILE
     *            connector with your parent Component. On Windows, make sure that you add enough slash characters at
     *            the beginning, for example: file:///c:/dir/file<br>
     * <br>
     *            If you serve files from a class loader, use clap:// URIs and make sure that you register a CLAP
     *            connector with your parent Component.<br>
     * <br>
     */
    ...
    /**
     * Returns the reference comparator used to sort index pages. The default implementation used a friendly alphanum
     * sorting.
     * 
